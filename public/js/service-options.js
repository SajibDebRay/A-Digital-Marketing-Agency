...
(function () {
  ...
  /* ── Base prices — must match BASE_PRICES in pricing.js ── */
  const BASE = {
  ...
  const sharedCSS = `
    ...
    .so-label {
      ...
      color: #e8c97a; margin-bottom: 0.4rem; letter-spacing: 0.03em;
    }
    .so-input, .so-textarea, .so-select {
      width: 100%; background: rgba(11,20,38,0.7);
      border: 1px solid rgba(201,168,76,0.25); border-radius: 8px;
      color: #f7f3ec; font-family: 'Jost', sans-serif; font-size: 0.88rem;
    ...
    .so-hint { font-size: 0.75rem; color: #8a96b0; margin-top: 0.35rem; line-height: 1.5; }
    .so-alert {
      background: rgba(201,168,76,0.1); border: 1px solid rgba(201,168,76,0.3);
      border-radius: 8px; padding: 0.75rem 1rem; font-size: 0.8rem;
      color: #e8c97a; line-height: 1.6; margin-bottom: 1rem;
    ...
    .so-price-preview {
      ...
      color: #f7f3ec; margin-bottom: 1.2rem;
      display: flex; justify-content: space-between; align-items: center;
    }
    .so-price-preview span:last-child {
      font-family: 'Cormorant Garamond', serif; font-size: 1.3rem;
      color: #e8c97a; font-weight: 500;
    }
    .so-add-btn {
      display: inline-flex; align-items: center; gap: 6px;
      padding: 0.5rem 1.1rem; background: rgba(201,168,76,0.1);
      border: 1px solid rgba(201,168,76,0.3); border-radius: 7px;
      color: #e8c97a; font-size: 0.78rem; font-weight: 500;
    ...
    .so-remove-btn {
      ...
      padding: 0.38rem 0.8rem; background: rgba(224,112,112,0.1);
      border: 1px solid rgba(224,112,112,0.25); border-radius: 6px;
      color: #e07070; font-size: 0.75rem; cursor: pointer;
      font-family: 'Jost', sans-serif; transition: background 0.2s; margin-top: 0.4rem;
    }
    ...
    .so-smm-platform { display: flex; gap: 0.6rem; flex-wrap: wrap; margin-bottom: 1rem; }
    .so-smm-chip {
      display: flex; align-items: center; gap: 7px;
      padding: 0.5rem 1.1rem; border: 1px solid rgba(201,168,76,0.25);
      border-radius: 20px; background: transparent; color: #8a96b0;
      font-size: 0.8rem; font-family: 'Jost', sans-serif; cursor: pointer; transition: all 0.2s;
    }
    .so-smm-chip.active {
      background: rgba(201,168,76,0.12); border-color: rgba(201,168,76,0.5); color: #e8c97a;
    }
    .so-smm-chip i { font-size: 1rem; }
    .so-divider { height: 1px; background: rgba(201,168,76,0.12); margin: 1.2rem 0; }
    .so-qty-row { display: flex; align-items: center; gap: 10px; }
    .so-qty-btn {
      width: 34px; height: 34px; border-radius: 7px;
      border: 1px solid rgba(201,168,76,0.3); background: rgba(201,168,76,0.08);
      color: #e8c97a; font-size: 1.1rem; cursor: pointer;
      display: flex; align-items: center; justify-content: center;
      flex-shrink: 0; transition: background 0.2s;
    }
    .so-qty-btn:hover { background: rgba(201,168,76,0.18); }
    .so-qty-display { font-size: 1rem; font-weight: 600; color: #f7f3ec; min-width: 60px; text-align: center; }
  `;
  ...
  function getContainer() { return document.getElementById('service-options-container'); }
  ...
  function buildEmailMarketing(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-price-preview"><span>Price per sending</span><span>${fmt(base)}</span></div>
        <div class="so-group">
        ...
        <div class="so-group">
          <label class="so-label">Email Subject Line</label>
        ...
        <div class="so-group">
          ...
          <div class="so-hint">Our team will design the HTML template based on your brief.</div>
        </div>
        <div class="so-alert">
  ...
  function buildDataScraping(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-price-preview"><span>Price per 1,000 emails</span><span>${fmt(base)}</span></div>
        <div class="so-group">
        ...
        <div class="so-group">
          ...
          <textarea class="so-textarea" id="ds-niche" placeholder="Describe the type of contacts you need — industry, job titles, company size, country, platforms (LinkedIn, websites, etc.)." maxlength="1500"></textarea>
  ...
  function buildSEO(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-alert">
          ...
          <span><strong>Important:</strong> Please contact our team before placing your SEO order to get a proper plan tailored to your website. Placing an order without consultation may lead to a mismatch of expectations.</span>
        </div>
        <div class="so-price-preview"><span>Price per website</span><span>${fmt(base)}</span></div>
        <div class="so-group">
          <label class="so-label">Website URLs</label>
          <div id="seo-url-list"></div>
          <button class="so-add-btn" id="seo-add-url"><i class="fas fa-plus"></i> Add Another Website</button>
          <div class="so-hint">Each additional website = +${fmt(base)}.</div>
        </div>
      </div>`;
    ...
    function renderUrls() {
      ...
      list.querySelectorAll('.seo-url-input').forEach((inp, i) => { inp.addEventListener('input', () => { urls[i] = inp.value; }); });
      list.querySelectorAll('.seo-remove').forEach(btn => {
        btn.addEventListener('click', () => { urls.splice(parseInt(btn.dataset.i), 1); renderUrls(); updatePrice(base * urls.length); });
      });
      updatePrice(base * (urls.length || 1));
    }
    document.getElementById('seo-add-url').addEventListener('click', () => { urls.push(''); renderUrls(); });
    renderUrls();
  }
  ...
  function buildAdminSupport(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-price-preview"><span>Price per day (5 hrs)</span><span>${fmt(base)}</span></div>
        <div class="so-group">
          ...
          <div class="so-hint">Each day = 5 hours at ${fmt(base)}/day.</div>
        </div>
        <div class="so-group">
        ...
        <div class="so-group">
          ...
          <textarea class="so-textarea" id="as-desc" placeholder="Describe the tasks you need completed — tools, platforms, access requirements, deadlines." maxlength="1000"></textarea>
  ...
  function buildDatabase(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-price-preview"><span>Price per 1,000 contacts</span><span>${fmt(base)}</span></div>
        <div class="so-group">
        ...
        <div class="so-group">
          ...
          <textarea class="so-textarea" id="db-niche" placeholder="Describe the niche or industry — e.g. real estate agents in the USA, e-commerce store owners in the UK, healthcare professionals, etc." maxlength="1500"></textarea>
        </div>
        <div class="so-group">
  ...
  function buildSMM(base) {
    ...
    getContainer().innerHTML = `
      <div class="so-wrap">
        ...
        <div class="so-price-preview"><span>Price per account/page</span><span>${fmt(base)}</span></div>
        <div class="so-group">
          ...
          <div class="so-hint">Each platform = ${fmt(base)}. Each additional URL = another account = +${fmt(base)}.</div>
        </div>
    ...
    function renderPlatformUrls() {
      ...
      selected.forEach(platform => {
        ...
        function renderUrls() {
          ...
          list.querySelectorAll('.smm-url-inp').forEach((inp, i) => { inp.addEventListener('input', () => { p.urls[i] = inp.value; }); });
          list.querySelectorAll('.smm-remove-url').forEach(btn => {
            btn.addEventListener('click', () => { p.urls.splice(parseInt(btn.dataset.i), 1); renderUrls(); updatePrice(base * countTotalAccounts()); });
          });
        }
        renderUrls();
        wrap.querySelector('.smm-add-url').addEventListener('click', () => { p.urls.push(''); renderUrls(); updatePrice(base * countTotalAccounts()); });
      });
    ...
    document.querySelectorAll('.so-smm-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        if (selected.has(platform)) { selected.delete(platform); platforms[platform].urls = []; chip.classList.remove('active'); }
        else { selected.add(platform); chip.classList.add('active'); }
        renderPlatformUrls();
      });
    });
  ...
  function init() {
    ...
    const serviceName = params.get('service') || localStorage.getItem('selectedService') || '';
    ...
    switch (serviceName) {
      case 'Email Marketing'        : buildEmailMarketing(base); break;
      case 'Data Scraping'          : buildDataScraping(base);   break;
      case 'SEO Services'           : buildSEO(base);            break;
      case 'Administrative Support' : buildAdminSupport(base);   break;
      case 'Database'               : buildDatabase(base);       break;
      case 'SMM Services'           : buildSMM(base);            break;
    }
  }
